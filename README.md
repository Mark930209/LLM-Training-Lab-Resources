...
课程编号是知识库位置，不是“前面所有包都必须复制”的意思。核心课程按下面的顺序累积：

```text
02 → 03 → 04 → 05 → 06 → 07 → 08 → 10
→ 11 → 12 → 14 → 15 → 16 → 29 → 31 → 33
```

09、13、17～28、30、32、34～39 属于高级分支，选择对应分支时再加入。后续核心模块不得反向 import 被跳过的分支模块；确需复用的底层能力应下沉到共享接口。当前尚未制作的包继续按这条依赖规则实现。

现有资源仍采用手工累积模块的方式，这次课程重组不改代码目录。把各里程碑改成一个可 clone、可切换版本的学习工程，列在 `Docs/operations/curriculum-rollout-recommendations.md`，作为后续独立任务执行。
