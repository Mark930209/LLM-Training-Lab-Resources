...
对应文章：`Articles/Season3/11_两台机器各有一块卡，怎样让它们真正一起训练？.md`

## 本篇解决什么问题

两台各带一块消费级 GPU 的 Windows 机器，想把它们凑成一个双卡训练环境。装好 PyTorch、写好 `torchrun` 命令之后，两个进程连 rendezvous 都完成不了。WSL2 的网络到底挡住了什么，要怎样才能让两块卡真正交换梯度？

核心判断：跨机 NCCL 的障碍不在 PyTorch 配置，而在网络地址。NCCL 的 bootstrap 阶段让各 rank 互相通告自己的真实 socket 地址，随后按这些地址直连建立数据面，端口由 NCCL 动态协商。只要 WSL2 处在 WinNAT 后面，出站流量就会被 SNAT 改写成宿主 Windows 的 LAN 地址，对端拿到的地址是假的，数据面必然建不起来。
...
| ④ Win11 mirrored 模式 | 全项通过，约 920 Mbps | 全部脚本（`.wslconfig` 配置见文章 §6.2） |
...
- 两侧运行时 NCCL 版本必须一致；不一致时按文章 §4.3 的三个坑对齐（`LD_PRELOAD` 是唯一有效手段）。
...
改完执行 `wsl --shutdown` 重启 WSL 生效。干净环境（防火墙开启）下还需放行 Hyper-V 防火墙入站，命令见文章 §6.3。
